...
import { extractItems, extractPagedList } from '../utils/api-response.util';
...
export class MachineApiService {
  ...
  getAll(params?: any): Observable<any[]> {
    ...
    if (params) Object.keys(params).forEach(k => { if (params[k] != null) p = p.set(k, params[k]); });
    return this.http.get<ApiResponse<any>>(`${API}/machines`, { params: p }).pipe(
      map(extractItems)
    );
  }
  getById(id: string): Observable<ApiResponse<any>> {
  ...
  create(data: MachineCreatePayload): Observable<ApiResponse<any>> {
    const body: MachineCreatePayload = {
      name: data.name,
    ...
    return this.http.post<ApiResponse<any>>(`${API}/machines`, body, {
...
export class AttendanceApiService {
  ...
  getStats(): Observable<ApiResponse<any>> {
    return this.http.get<ApiResponse<any>>(`${API}/attendance/stats`);
  }
  getAll(params?: any): Observable<any[]> {
    let p = new HttpParams();
    if (params) Object.keys(params).forEach(k => { if (params[k] != null) p = p.set(k, params[k]); });
...
export class ProgressApiService {
  ...
  deleteWorkoutLog(id: string): Observable<ApiResponse<any>> {
    return this.http.delete<ApiResponse<any>>(`${API}/progress/${id}`);
  }
}
...
export class WorkoutPlanApiService {
  ...
  generate(data: { goal: string; level: string }): Observable<ApiResponse<any>> {
    return this.http.post<ApiResponse<any>>(`${API}/workout-plans/generate`, data);
  }
  update(id: string, data: any): Observable<ApiResponse<any>> {
...
export class TrainerApiService {
  ...
  getAllPaged(params?: any): Observable<{ items: any[]; total: number }> {
    let p = new HttpParams().set('limit', '100');
    if (params) Object.keys(params).forEach(k => { if (params[k] != null) p = p.set(k, params[k]); });
    return this.http.get<ApiResponse<any>>(`${API}/trainers`, { params: p }).pipe(
      map(extractPagedList)
    );
  }
  getById(id: string): Observable<ApiResponse<any>> {
  ...
  approve(id: string): Observable<ApiResponse<any>> {
    return this.http.patch<ApiResponse<any>>(`${API}/trainers/${id}`, { isApproved: true });
  }
  reject(id: string): Observable<ApiResponse<any>> {
    return this.http.patch<ApiResponse<any>>(`${API}/trainers/${id}`, { isApproved: false });
  }
  setAvailability(id: string, isAvailable: boolean): Observable<ApiResponse<any>> {
    return this.http.patch<ApiResponse<any>>(`${API}/trainers/${id}`, { isAvailable });
  }
  create(data: any): Observable<ApiResponse<any>> {
    return this.http.post<ApiResponse<any>>(`${API}/trainers`, data);
  }
  createForUser(userId: string, data: any): Observable<ApiResponse<any>> {
    return this.http.post<ApiResponse<any>>(`${API}/trainers/for-user/${userId}`, data);
  }
  delete(id: string): Observable<ApiResponse<any>> {
    return this.http.delete<ApiResponse<any>>(`${API}/trainers/${id}`);
  }
  assign(trainerId: string, userId: string): Observable<ApiResponse<any>> {
    return this.http.patch<ApiResponse<any>>(`${API}/trainers/${trainerId}/assign/${userId}`, {});
  }
  unassign(trainerId: string, userId: string): Observable<ApiResponse<any>> {
    return this.http.patch<ApiResponse<any>>(`${API}/trainers/${trainerId}/unassign/${userId}`, {});
  }
}
...
export class AdminApiService {
  ...
  getAllUsers(params?: any): Observable<any[]> {
    let p = new HttpParams().set('limit', '100');
    if (params) Object.keys(params).forEach(k => { if (params[k] != null) p = p.set(k, params[k]); });
    return this.http.get<ApiResponse<any>>(`${API}/users`, { params: p }).pipe(
      map(extractItems)
    );
  }
  getAllUsersPaged(params?: any): Observable<{ items: any[]; total: number }> {
    let p = new HttpParams().set('limit', '100');
    if (params) Object.keys(params).forEach(k => { if (params[k] != null) p = p.set(k, params[k]); });
    return this.http.get<ApiResponse<any>>(`${API}/users`, { params: p }).pipe(
      map(extractPagedList)
  ...
  deactivateUser(id: string): Observable<ApiResponse<any>> {
    return this.http.patch<ApiResponse<any>>(`${API}/users/${id}/deactivate`, {});
  }
  activateUser(id: string): Observable<ApiResponse<any>> {
    return this.http.patch<ApiResponse<any>>(`${API}/users/${id}/activate`, {});
  }
  getAllAttendance(params?: any): Observable<any[]> {
    let p = new HttpParams().set('limit', '100').set('sort', '-checkIn');
    if (params) Object.keys(params).forEach(k => { if (params[k] != null) p = p.set(k, params[k]); });
    return this.http.get<ApiResponse<any>>(`${API}/attendance`, { params: p }).pipe(
      map(extractItems)
    );
  ...
  getAllSubscriptionsPaged(params?: any): Observable<{ items: any[]; total: number }> {
    let p = new HttpParams().set('limit', '100');
    if (params) Object.keys(params).forEach(k => { if (params[k] != null) p = p.set(k, params[k]); });
    return this.http.get<ApiResponse<any>>(`${API}/subscriptions`, { params: p }).pipe(
      map(extractPagedList)
